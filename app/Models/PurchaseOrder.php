...
class PurchaseOrder extends Model
{
    ...
    {
        return $this->belongsTo(Customer::class, 'id_supplier', 'id');
    }

    public function relasi()
    {
        if ($this->tipe_po === 'penjualan') {
    ...
    {
        return $this->hasMany(Retur::class, 'id_sumber', 'id_po')
                    ->where('tipe_retur', 'po');
    }
    ...
    public function needsReceiptConfirmation()
    {
        ...
        if ($this->tipe_po === 'penjualan' && $this->status === 'selesai' && !$this->tanggal_diterima) {
            return true;
        }

        // Eksternal: ada shipping activity "diterima" atau "selesai" tapi belum dikonfirmasi
        if ($this->tipe_po === 'pembelian' && !$this->tanggal_diterima) {
