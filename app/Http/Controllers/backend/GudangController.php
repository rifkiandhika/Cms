...
class GudangController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            // Detail barang fields
            'kode_produk.*' => 'nullable|string|max:100',
            'nama_dagangan.*' => 'nullable|string|max:255',
            'nomor_izin_edar.*' => 'nullable|string|max:100',
            'tipe.*' => 'nullable|string|max:100',
            'ukuran.*' => 'nullable|string|max:100',
            'kemasan.*' => 'nullable|string|max:100',
            'satuan.*' => 'nullable|string|max:50',
            'satuan_lain.*' => 'nullable|string|max:100',
            ...
            if ($request->has('barang_id') && is_array($request->barang_id)) {
                foreach ($request->barang_id as $index => $barangId) {
                    DetailGudang::create([
                        ...
                        'kode_produk' => $request->kode_produk[$index] ?? null,
                        'nama_dagangan' => $request->nama_dagangan[$index] ?? null,
                        'nomor_izin_edar' => $request->nomor_izin_edar[$index] ?? null,
                        'tipe' => $request->tipe[$index] ?? null,
                        'ukuran' => $request->ukuran[$index] ?? null,
                        'kemasan' => $request->kemasan[$index] ?? null,
                        'satuan' => $request->satuan[$index] ?? null,
                        'satuan_lain' => $request->satuan_lain[$index] ?? null,
                        'kode_barcode' => $request->kode_barcode[$index] ?? null,
                        ...
                        'jumlah_keluar' => $request->jumlah_keluar[$index] ?? 0,
                        'jumlah_retur' => $request->jumlah_retur[$index] ?? 0,
    ...
    public function detailsData($id)
    {
        ...
        return DataTables::of($details)
            ...
            ->addColumn('barang_nama', function ($detail) {
                return match ($detail->barang_type) {
                    'obat'       => $detail->barangObat->nama_obat_rs ?? '-',
                    'alkes'      => $detail->alkes->nama_alkes ?? '-',
                    'reagensia'  => $detail->reagensia->nama_reagensia ?? '-',
                    default      => '-',
                };
            })

            ->addColumn('nama_dagangan', function ($detail) {
                return $detail->nama_dagangan ?? '-';
            })

            ->addColumn('nomor_izin_edar', function ($detail) {
                return $detail->nomor_izin_edar ?? '-';
            })
            ...
            ->addColumn('ukuran', function ($detail) {
                return $detail->ukuran ?? '-';
            })

            ->addColumn('kemasan', function ($detail) {
                return $detail->kemasan ?? '-';
            })
            ...
            ->addColumn('satuan_lain', function ($detail) {
                return $detail->satuan_lain ?? '-';
            })

            ->addColumn('kode_barcode', function ($detail) {
                return $detail->kode_barcode ?? '-';
            })

            ->addColumn('jumlah_stock', function ($detail) {
                return $detail->stock_gudang ?? 0;
            })

            ->addColumn('jumlah_keluar', function ($detail) {
                return $detail->jumlah_keluar ?? 0;
            })

            ->addColumn('jumlah_retur', function ($detail) {
                return $detail->jumlah_retur ?? 0;
            })
            ...
            ->editColumn('barang_type', function ($detail) {
                ...
                return $badges[$detail->barang_type]
