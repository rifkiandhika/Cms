        ...
        <div class="card border-0 shadow-sm h-100">
            <div class="card-body d-flex align-items-center">
                ...
                <div>
                    ...
                    <h4 class="mb-0">{{ $pengendalianHamaList->count() }}</h4>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card border-0 shadow-sm h-100">
            <div class="card-body d-flex align-items-center">
                ...
                <div>
                    ...
                    <h4 class="mb-0">{{ $pengendalianHamaList->sum('details_count') }}</h4>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card border-0 shadow-sm h-100">
            <div class="card-body d-flex align-items-center">
                ...
                <div>
                    ...
                    <h4 class="mb-0">{{ $pengendalianHamaList->sum(fn($d) => $d->gambar->count()) }}</h4>
                </div>
            </div>
        </div>
    </div>
...
<div class="card border-0 shadow-sm">
    ...
    <div class="card-body border-bottom bg-light">
        ...
        <input type="text" id="searchHama" class="form-control form-control-sm"
               placeholder="Cari lokasi, bulan, tahun...">
    </div>

    <div class="card-body">
        <div>
            @if($pengendalianHamaList->count() > 0)
                <table class="table table-hover table-responsive" id="hamaPartialTable">
                    ...
                    <tbody>
                        @foreach($pengendalianHamaList as $x => $item)
                        <tr>
                            ...
                            <td class="text-center">
                                <div class="btn-group">
                                    <a href="{{ route('pengendalian-hama.edit', $item->id) }}" 
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        ...
                                        <li>
                                            <a href="{{ route('pengendalian-hama.export-pdf', $item->id) }}" 
                                                class="dropdown-item" 
                                                target="_blank">
                                        ...
                                        <li>
                                            <form action="{{ route('pengendalian-hama.destroy', $item->id) }}"
                                                  method="POST" class="delete-hama-confirm">
                                                @csrf @method('DELETE')
...
$(document).ready(function () {
    let hamaTable = $('#hamaPartialTable').DataTable({
        dom: 'rtip',
        pageLength: 10,
        responsive: true,
    ...
    $('#searchHama').on('keyup', function () {
        hamaTable.search(this.value).draw();
    });

    $(document).on('submit', '.delete-hama-confirm', function (e) {
        e.preventDefault();
        var form = this;
        Swal.fire({
            title: 'Hapus Laporan?',
            text: 'Semua catatan harian dan foto akan ikut terhapus!',
            icon: 'warning',
