        ...
        if (items.length > 0) {
            items.forEach(item => {
                ...
                if (tipe === 'po') {
                    ...
                    // ✅ PERUBAHAN: Gunakan no_gr - no_invoice sebagai kode referensi
                    const noGr = item.no_gr || '-';
                    const noInvoice = item.no_invoice || '-';
                    dataKode = `${noGr} - ${noInvoice}`;
                    
                    // ✅ PERUBAHAN: Tampilkan format baru di dropdown
                    const supplierName = item.supplier?.nama_supplier || item.supplier?.nama || 'Supplier';
                    optionText = `GR: ${noGr} | Invoice: ${noInvoice} | ${supplierName} | ${formatTanggal(item.tanggal_permintaan) || ''}`;
