@extends('layouts.app')
@section('title', 'Edit Daftar Hadir Pelatihan')
...
<section>
    <div class="container-fluid">
        <div class="d-sm-flex mb-3">
            <h1 class="h3 mb-0 text-gray-600">Edit Formulir Daftar Hadir Pelatihan</h1>
        </div>

        <form action="{{ route('attendance-forms.update', $attendanceForm->id) }}" method="POST" id="attendanceForm">
            @csrf @method('PUT')
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-9">
                            <input type="text" name="topik_pelatihan" class="form-control"
                                   value="{{ old('topik_pelatihan', $attendanceForm->topik_pelatihan) }}" required>
                        </div>
                    </div>
                    <div class="row mb-3">
                        ...
                        <div class="col-md-9">
                            <input type="date" name="tanggal" class="form-control"
                                   value="{{ old('tanggal', $attendanceForm->tanggal?->format('Y-m-d')) }}">
                        </div>
                    </div>
                    <div class="row mb-3">
                        ...
                        <div class="col-md-9">
                            <input type="text" name="tempat" class="form-control"
                                   value="{{ old('tempat', $attendanceForm->tempat) }}">
                        </div>
                    </div>
                    <div class="row mb-3">
                        ...
                        <div class="col-md-9">
                            <input type="text" name="instruktur" class="form-control"
                                   value="{{ old('instruktur', $attendanceForm->instruktur) }}">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card shadow-sm border-0 mb-4">
                <div class="card-header bg-white py-3">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <small class="text-muted">Kolom yang sudah ada otomatis dimuat. Klik "+ Kolom" untuk menambah kolom baru.</small>
                        </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered align-middle mb-0" id="participantsTable">
                            ...
                            <tbody id="participantsBody">
                                @foreach($attendanceForm->participants as $pi => $p)
                                <tr class="participant-row">
                                    <td class="text-center row-number">{{ $pi + 1 }}</td>
                                    <td><input type="text" name="participants[{{ $pi }}][nama_karyawan]" class="form-control form-control-sm" value="{{ $p->nama_karyawan }}"></td>
                                    <td><input type="text" name="participants[{{ $pi }}][jabatan]" class="form-control form-control-sm" value="{{ $p->jabatan }}"></td>
                                    <td><input type="text" name="participants[{{ $pi }}][lokasi_kerja]" class="form-control form-control-sm" value="{{ $p->lokasi_kerja }}"></td>
                                    @foreach($attendanceForm->getCustomColumnLabels() as $ci => $label)
                                        <td class="custom-td" data-col-index="{{ $ci }}">
                                            <input type="text" name="participants[{{ $pi }}][custom][{{ $ci }}]"
                                                   class="form-control form-control-sm"
                                                   value="{{ $p->custom_values[$ci] ?? '' }}"
                                                   placeholder="{{ $label }}">
                                        </td>
                                    @endforeach
                ...
                {{-- Panel Kolom --}}
                <div class="card-footer bg-light" id="customColumnsPanel"
                     style="{{ $attendanceForm->getCustomColumnLabels() ? 'display:block' : 'display:none' }}">
            ...
            <div class="card shadow-sm border-0 mb-4">
                <div class="card-body">
                    ...
                    <textarea name="catatan" class="form-control" rows="3">{{ old('catatan', $attendanceForm->catatan) }}</textarea>
                </div>
            </div>

            <div class="d-flex justify-content-between mb-4">
                <a href="{{ route('attendance-forms.show', $attendanceForm->id) }}" class="btn btn-secondary btn-sm">
...
$(document).ready(function () {
    let participantIndex = {{ $attendanceForm->participants->count() }};

    // Muat kolom yang sudah ada dari DB
    let customCols = @json($attendanceForm->getCustomColumnLabels()).map((label, i) => ({ label, index: i }));
    ...
    $(document).on('click', '.btn-remove-col', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                if ($('#customColumnTags .col-tag').length === 0) $('#customColumnsPanel').hide();
            }
        });
    });
    ...
    function updateRowNumbers() {
        $('.participant-row').each(function (i) { $(this).find('.row-number').text(i + 1); });
    }
