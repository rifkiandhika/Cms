        ...
        <div class="card shadow-sm border-0">
            <div class="card-header bg-white py-3">
                <div class="d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">
                        ...
                        @if(str_contains($tabType, 'draft'))
                            Daftar Tagihan Draft {{ $tipeRelasi === 'customer' ? 'Customer' : 'Supplier' }}
                        @elseif(str_contains($tabType, 'lunas'))
                            Daftar Tagihan Lunas {{ $tipeRelasi === 'customer' ? 'Customer' : 'Supplier' }}
                        @else
                            Daftar Tagihan {{ $tipeRelasi === 'customer' ? 'Customer' : 'Supplier' }}
                        @endif
                    </h5>
                </div>
            </div>
            ...
            <div class="card-body border-bottom bg-light">
                <form method="GET" action="{{ route('tagihan.index') }}">
                    ...
                    <div class="row g-3">
                        <!-- Filter Relasi (Supplier/Customer) -->
                        <div class="col-md-{{ str_contains($tabType, 'aktif') ? '3' : '4' }}">
                            <label class="form-label small fw-bold">{{ $tipeRelasi === 'customer' ? 'Customer' : 'Supplier' }}</label>
                            <select name="relasi_id" class="form-select form-select-sm">
                                <option value="">Semua {{ $tipeRelasi === 'customer' ? 'Customer' : 'Supplier' }}</option>
                                @foreach($relasi as $r)
                                    <option value="{{ $r->id }}" {{ request('relasi_id') == $r->id ? 'selected' : '' }}>
                                        {{ $tipeRelasi === 'customer' ? $r->nama_customer : $r->nama_supplier }}
                                    </option>
                                @endforeach
                            </select>
                        </div>

                        @if(str_contains($tabType, 'aktif'))
                        <!-- Filter Status -->
                        <div class="col-md-2">
                        ...
                        <div class="col-md-{{ str_contains($tabType, 'aktif') ? '3' : '4' }}">
                            ...
                            <input type="text" class="form-control form-control-sm" name="search" 
                                   placeholder="Cari No Tagihan, No PO..." value="{{ request('search') }}">
                        </div>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-striped" id="tagihanTable-{{ $tabType }}">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th>No PO/GR</th>
                                <th>No Invoice</th>
                                <th>{{ $tipeRelasi === 'customer' ? 'Customer' : 'Supplier' }}</th>
                                <th>Tanggal</th>
                                @if(str_contains($tabType, 'aktif'))
                                <th>Jatuh Tempo</th>
                                @endif
                                @if(str_contains($tabType, 'lunas'))
                                <th>Tanggal Lunas</th>
                                @endif
                        ...
                        <tbody>
                                        ...
                                        <div class="d-flex align-items-center">
                                            <div class="avatar-xs me-2">
                                                <span class="avatar-title rounded-circle bg-soft-primary text-primary">
                                                    <i class="{{ $tipeRelasi === 'customer' ? 'ri-user-line' : 'ri-store-2-line' }}"></i>
                                                </span>
                                            </div>
                                            <div>
                                                <strong>
                            ...
                            @empty
                                <tr>
                                    <td colspan="{{ str_contains($tabType, 'aktif') ? 12 : (str_contains($tabType, 'lunas') ? 10 : 9) }}" class="text-center py-5">
