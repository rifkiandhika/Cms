            ...
            <div class="card shadow-sm border-0">
                <div class="card-header bg-white py-3">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div class="btn-group">
                            <a class="btn btn-primary btn-sm" href="{{ route('po.create', ['type' => 'penjualan']) }}">
                                <i class="ri-add-circle-line me-1"></i>Penjualan
                            </a>
                            <a class="btn btn-success btn-sm" href="{{ route('po.create', ['type' => 'pembelian']) }}">
                                <i class="ri-add-circle-line me-1"></i>Pembelian
                            </a>
                        </div>
                    </div>
                </div>
                    ...
                    <ul class="nav nav-tabs nav-tabs-custom" id="mainTabs" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button class="nav-link active" id="internal-tab" data-bs-toggle="tab" data-bs-target="#internal-content" 
                                ...
                                <i class="ri-home-line me-2"></i>Penjualan
                                <span class="badge bg-primary ms-2">{{ $purchaseOrders->where('tipe_po', 'penjualan')->count() }}</span>
                            </button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="external-tab" data-bs-toggle="tab" data-bs-target="#external-content" 
                                ...
                                <i class="ri-building-line me-2"></i>Pembelian
                                <span class="badge bg-success ms-2">{{ $purchaseOrders->where('tipe_po', 'pembelian')->count() }}</span>
                            </button>
                        </li>
                    </ul>
                </div>
                ...
                <div class="card-body">
                    <div class="tab-content" id="mainTabsContent">
                        ...
                        <div class="tab-pane fade show active" id="internal-content" role="tabpanel" aria-labelledby="internal-tab">
                            @include('po.partials.internal-tabs', ['purchaseOrders' => $purchaseOrders->where('tipe_po', 'penjualan')])
                        </div>

                        {{-- Tab External --}}
                        <div class="tab-pane fade" id="external-content" role="tabpanel" aria-labelledby="external-tab">
                            @include('po.partials.external-tabs', ['purchaseOrders' => $purchaseOrders->where('tipe_po', 'pembelian')])
                        </div>
                    </div>
                </div>
            </div>
