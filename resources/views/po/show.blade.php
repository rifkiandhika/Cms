                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <div>
                                ...
                                @else
                                    <strong class="text-primary">{{ ucfirst($po->customer->nama_customer) }}</strong>
                                @endif
                            </div>
                        </div>
                    </div>
            ...
            <div class="card shadow-sm border-0 mb-4 border-info" style="border-width: 2px !important;">
                <div class="card-header bg-info text-white py-3 d-flex justify-content-between align-items-center">
                    ...
                    <button type="button" class="btn btn-light btn-sm" onclick="showInvoiceProofModal()">
                        ...
                        {{ $po->hasInvoiceProof() ? 'Lihat Bukti Invoice & Barang': 'Upload Bukti Invoice & Barang' }} 
                    </button>
                </div>
                <div class="card-body">
                ...
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table class="table table-hover mb-0">
                            ...
                            <tfoot class="table-light">
                                    @php
                                        $pajak = !empty($po->pajak_diterima) && $po->pajak_diterima > 0
                                                    ? $po->pajak_diterima
                                                    : $po->pajak;

                                        $grand = !empty($po->grand_total_diterima) && $po->grand_total_diterima > 0
                                                    ? $po->grand_total_diterima
                                                    : $po->grand_total;
                                    @endphp
                                    ...
                                    <td>
                                        <span class="badge badge-sm bg-secondary">
                                            @php
                                                ...
                                                if ($po->tipe_po === 'penjualan' && str_contains($aksiText, 'approve_kepala_gudang')) {
                    ...
                    @php
                        $statusConfig = [
                            ...
                            'menunggu_persetujuan_kepala_gudang' => [
                                'color' => 'warning', 
                                'icon' => 'ri-time-line', 
                                'label' => $po->tipe_po === 'penjualan' ? 'Menunggu Persetujuan Gudang' : 'Menunggu Kepala Gudang'
                            ],
...
<script>
    ...
    function showApprovalModal(role, status) {
        ...
        const roleText = role === 'kepala_gudang' 
            ? (poType === 'penjualan' ? 'Gudang' : 'Kepala Gudang')
            : 'Kasir';
